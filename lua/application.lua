...
--- INTERVAL ---
-- In milliseconds, (x * 60000) where x = minutes . Remember that the sensor reading, 
-- reboot and wifi reconnect takes a few seconds
time_between_sensor_readings = 1*60000                               

-- All topics:  nodemcu/# 

-- Topic: nodemcu/ping Sends a simple ping to the broker
local function send_ping()  
    m:publish(config.ENDPOINT .. "ping","id=" .. config.ID,0,0)
end

-- Topic: nodemcu/temperature Sends a simple temperature to the broker
local function send_tmp_mqtt()
    m:publish(config.ENDPOINT .. "temperature","t=" .. t,0,0)
end
...
function sendData(temp)
    ...
    con:on("receive", function(conn, payload)success = true print(payload)end)
    con:on("connection",
    function(conn, payload)
    print("Connectado")
    con:send('GET /update?key='..WRITEKEY..'&field1='..temp..'HTTP/1.1\r\n\Host: api.thingspeak.com\r\nAccept: */*\r\nUser-Agent: Mozilla/4.0 (compatible; esp8266 Lua; Windows NT 5.1)\r\n\r\n')end)
    con:on("disconnection", function(conn, payload) print('Desconectado') end)
    con:connect(80,'184.106.153.149')
end
...
function module.start()  
  ...
  tmr.alarm(1,20000, 1, function() ds18b20_start() end)
